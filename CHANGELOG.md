...
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.8
- `#{{SHA_SHORT}}` - ### :sparkles: New Features remove 'Own Supabase' option from storage selector
- `#{{SHA_SHORT}}` - ### :sparkles: New Features add auto-update system with GitHub Releases integration
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.9
- `#{{SHA_SHORT}}` - security: remove legacy credentials.rs with insecure key derivation
- `#{{SHA_SHORT}}` - Merge remote-tracking branch 'kofany/main'
- `#{{SHA_SHORT}}` - ### :bug: Bug Fixes password dialog focus escape and OpenWRT shell prompt
- `#{{SHA_SHORT}}` - ### :sparkles: New Features improve changelog display in update dialog
- `#{{SHA_SHORT}}` - ### :wrench: Chores bump version to v0.2.10
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.10
- `#{{SHA_SHORT}}` - ### :construction_worker: CI/CD add macOS Intel build to release workflow
- `#{{SHA_SHORT}}` - Merge remote-tracking branch 'kofany/main'
- `#{{SHA_SHORT}}` - ### :sparkles: New Features display SSH auth banner (RFC 4252)
- `#{{SHA_SHORT}}` - ### :wrench: Chores bump version to v0.2.11
- `#{{SHA_SHORT}}` - ### :wrench: Chores update lockfiles
- `#{{SHA_SHORT}}` - ### :memo: Documentation update RELEASES.md for public repo builds
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.11
- `#{{SHA_SHORT}}` - ### :memo: Documentation update RELEASES.md with public repo workflow details
- `#{{SHA_SHORT}}` - ### :sparkles: New Features add disconnect dialog with reconnect functionality
- `#{{SHA_SHORT}}` - ### :wrench: Chores bump version to v0.2.12
- `#{{SHA_SHORT}}` - ### :wrench: Chores update russh to 0.57.0
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.12
- `#{{SHA_SHORT}}` - ### :wrench: Chores update dependencies and remove unused packages
- `#{{SHA_SHORT}}` - ### :wrench: Chores bump version to v0.2.13
- `#{{SHA_SHORT}}` - ### :memo: Documentation update CHANGELOG.md for v0.2.13
- `#{{SHA_SHORT}}` - ### :bug: Bug Fixes prevent shell injection in changelog workflow
- `#{{SHA_SHORT}}` - ### :construction_worker: CI/CD move workflows to public repo
- `#{{SHA_SHORT}}` - ### :sparkles: New Features replace theme arrows with dropdown picker
- `#{{SHA_SHORT}}` - ### :sparkles: New Features add searchable theme picker with live preview
- `#{{SHA_SHORT}}` - ### :wrench: Chores bump version to v0.2.14
...
-  - ### :memo: Documentation update CHANGELOG.md for v0.2.12
