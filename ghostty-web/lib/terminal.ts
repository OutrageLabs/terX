    ...
    try {
      ...
      const mouseConfig: MouseTrackingConfig = {
        ...
        getCanvasOffset: () => {
          ...
          const debug = (window as any).terxDebug?.canvasDebug;
          if (debug) {
      ...
      // NOTE: No SelectionManager is created - beamterm has built-in selection
      // that handles mouse events, rendering, and clipboard automatically
      // (enabled via renderer.enableSelection(SelectionMode.Block, true))
